...
function Joystick_OnMouseMove(event)
{
	...
	CallAgent(parseInt(_PositionX, 10), parseInt(_PositionY, 10));
}
...
function CallAgent(axisX, axisY)
{
	console.log(axisX + " : " + axisY);
	const Http = new XMLHttpRequest();
	const Url = "http://raspberrypi.local:8080/wheels";

	Http.open("PUT", Url, true);
    Http.setRequestHeader('Content-Type', 'application/json');
    Http.send(`{"YAxis":${axisY},"XAxis":${axisX}}`);
}
